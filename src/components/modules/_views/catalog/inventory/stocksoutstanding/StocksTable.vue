    ...
    <bs-table
      ...
      <tr
        ...
        <td>
          <div class="flex-container flex-container--wrap">
            ...
            <div v-else>
              ...
              <div v-if="item.product_variant_id !== null">
                <p v-if="item.product_variant_sku !== null || item.product_sku !== ''">{{ item.product_variant_name }} - {{ item.product_variant_sku }}</p>
                <p v-else>{{ item.product_variant_name }}</p>
              </div>
            </div>
          </div>
        ...
        <td class="text-right" style="padding-right: 12px !important">
          <template>
            {{ item.inoutitemsextref.qty_send }}
          </template>
        </td>
        <td class="text-right" style="padding-right: 12px !important">
          <template>
            {{ item.inoutitemsextref.qty_received }}
          </template>
        </td>
        <td class="text-right color-warning" style="padding-right: 12px !important">
          <template>
            {{ item.inoutitemsextref.qty_difference }}
          </template>
        </td>
        <!-- <td v-if="editable">
          <el-button
      ...
      <tr v-if="dataStock.status === 'P'">
        ...
        <td class="text-right" style="padding-right: 12px !important">{{ dataStock.total_qty_send }}</td>
        <td class="text-right" style="padding-right: 12px !important">{{ dataStock.total_qty_received }}</td>
        <td class="text-right color-warning" style="padding-right: 12px !important">{{ dataStock.total_qty_difference }}</td>
      </tr>
    </bs-table>
  ...
  computed: {
    ...
    computedHeaderTable() {
      const header = [
        '', 
        this.lang.name, 
        this.lang.qty + ' ' + this.lang.total, 
        this.lang.qty + ' ' + this.rootLang.accepted,
        this.lang.qty + ' ' + this.lang.difference]
      if (!this.editable) {
