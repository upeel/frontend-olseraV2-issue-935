    ...
    <div class="main-content" shadow="never">
      <div class="table-handler-flex">
        <div style="flex-grow: 1;">
          <h4 class="main-content__title">{{ $lang[langId].list }} {{ lang.product }} {{ $lang[langId].raw_material }}</h4>
          <p class="main-content__subtitle">{{ params.total }} {{ lang.product }}</p>
        </div>
        <div>
          <button-action-authenticated
            :permission="['catalog/rawmaterial', 'store']"
            type="primary"
            icon="el-icon-plus"
      ...
      <div>
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <button-action-authenticated
                :permission="['catalog/rawmaterial', 'destroy']"
                :disabled="disabledButton"
                type="danger"
                class="btn-plain"
...
export default {
  ...
  methods: {
    getData: function() {
      ...
      axios({
        method: 'GET',
        url: baseApi(this.selectedStore.url_id, this.langId, 'product-mentah'),
        params: this.params,
        headers: headers
      }).then(response => {
    ...
    addData() {
      ...
      axios({
        method: 'POST',
        url: baseApi(this.selectedStore.url_id, this.langId, 'product-mentah/add'),
        data,
        headers: headers
      }).then(response => {
    ...
    removeItem(id) {
      ...
      axios({
        method: 'DELETE',
        url: baseApi(this.selectedStore.url_id, this.langId, 'product-mentah/destroy/' + id),
        headers: headers
      }).then(response => {
        this.disabledButton = false
        this.$message({
