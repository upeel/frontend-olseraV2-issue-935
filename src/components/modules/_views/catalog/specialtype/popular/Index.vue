    ...
    <div class="main-content" shadow="never">
      <div slot="header" class="table-handler-flex">
        <div style="flex-grow: 1;">
          <h4 class="main-content__title">{{ this.$lang[langId].list }} {{ lang.popular_product }}</h4>
          <p class="main-content__subtitle">{{ params.total }} {{ lang.product }}</p>
        </div>
        <div>
          <button-action-authenticated :permission="['catalog/popular', 'store']"
            type="primary"
            icon="el-icon-plus"
      ...
      <div class="">
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <button-action-authenticated :permission="['catalog/popular', 'destroy']" type="danger" class="btn-plain"
                :disabled="disabledButton"
...
export default {
  ...
  methods: {
    getData: function() {
      ...
      axios({
        method: 'GET',
        url: baseApi(this.selectedStore.url_id, this.langId, 'product-popular'),
        params: this.params,
        headers: headers
      }).then(response => {
    ...
    addData() {
      ...
      axios({
        method: 'POST',
        url: baseApi(this.selectedStore.url_id, this.langId, 'product-popular/add'),
        data,
        headers: headers
      }).then(response => {
    ...
    removeItem(id) {
      ...
      axios({
        method: 'DELETE',
        url: baseApi(this.selectedStore.url_id, this.langId, 'product-popular/destroy/' + id),
        headers: headers
      }).then(response => {
        this.disabledButton = false
        this.$message({
