    ...
    <div class="main-content" shadow="never">
      <div slot="header" class="table-handler-flex">
        <div style="flex-grow: 1;">
          <h4 class="main-content__title">{{ $lang[langId].list }} {{ lang.product }} {{ lang.out_of_stock }}</h4>
          <p class="main-content__subtitle">{{ params.total }} {{ lang.product }}</p>
        </div>
        <div>
          <button-action-authenticated
            :permission="['catalog/outstock', 'store']"
            type="primary"
            icon="el-icon-plus"
      ...
      <div class="">
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <button-action-authenticated :permission="['catalog/outstock', 'destroy']" type="danger" class="btn-plain"
                :disabled="disabledButton"
...
export default {
  ...
  methods: {
    getData() {
      ...
      axios({
        method: 'GET',
        url: baseApi(this.selectedStore.url_id, this.langId, 'product-outstock'),
        params: this.params,
        headers: headers
      }).then(response => {
    ...
    searchProducts(query) {
      ...
      if (query.length > 0) {
        axios({
          method: 'GET',
          url: baseApi(this.selectedStore.url_id, this.langId, 'product-outstock/getproduct'),
          headers: headers,
          params: params
        }).then(response => {
    ...
    addData() {
      ...
      axios({
        method: 'POST',
        url: baseApi(this.selectedStore.url_id, this.langId, 'product-outstock/add'),
        data,
        headers: headers
      }).then(response => {
    ...
    removeItem(id) {
      ...
      axios({
        method: 'DELETE',
        url: baseApi(this.selectedStore.url_id, this.langId, 'product-outstock/destroy/' + id),
        headers: headers
      }).then(response => {
        this.disabledButton = false
        this.$message({
