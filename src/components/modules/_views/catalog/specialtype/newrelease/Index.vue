    ...
    <div class="main-content" shadow="never">
      <div class="table-handler-flex">
        <div style="flex-grow: 1;">
          <h4 class="main-content__title">{{ $lang[langId].list }} {{ lang.new_release_product }}</h4>
          <p class="main-content__subtitle">{{ params.total }} {{ lang.product }}</p>
        </div>
        <div>
          <button-action-authenticated
            :permission="['catalog/newrelease', 'store']"
            type="primary"
            icon="el-icon-plus"
      ...
      <div class="">
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <button-action-authenticated :permission="['catalog/newrelease', 'destroy']" type="danger" class="btn-plain"
                :disabled="disabledButton"
...
export default {
  ...
  methods: {
    getData() {
      ...
      axios({
        method: 'GET',
        url: baseApi(this.selectedStore.url_id, this.langId, 'product-rilis-terbaru'),
        params: this.params,
        headers: headers
      }).then(response => {
    ...
    addData() {
      ...
      axios({
        method: 'POST',
        url: baseApi(this.selectedStore.url_id, this.langId, 'product-rilis-terbaru/add'),
        data,
        headers: headers
      }).then(response => {
    ...
    removeItem(id) {
      ...
      axios({
        method: 'DELETE',
        url: baseApi(this.selectedStore.url_id, this.langId, 'product-rilis-terbaru/destroy/' + id),
        headers: headers
      }).then(response => {
        this.disabledButton = false
        this.$message({
