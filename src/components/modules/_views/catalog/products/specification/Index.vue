<template>
  <div>
    <div class="affix-wrapper flex-container flex-container--start flex-container--desktop">
      <div
        v-if="checkCustomPermission('catalog/specifications', 'store')"
      ...
      <el-card v-loading="loading" shadow="never" class="flex-grow-1">
        <div slot="header" class="table-handler-flex">
          <h4 style="flex-grow: 1;">{{ $lang[langId].list }} {{ lang.specification }}</h4>
          <div v-if="showSaveSorts">
            <button-action-authenticated
              :permission="['catalog/specifications', 'edit']"
        ...
        <div class="card-body">
          <draggable
            :options="{group:{ name:'specifications'}}"
            :list="data"
            handle=".hand"
            ...
            <div
              ...
              <draggable
                ...
                <div
                  ...
                  class="dd-item"
                >
                  <list :item="child" @edit="handleEditItem" />
                </div>
              </draggable>
...
export default {
  ...
  methods: {
    ...
    save(data) {
      ...
      axios({
        method: 'POST',
        url: baseApi(this.selectedStore.url_id, this.langId, 'specification/store'),
        headers: headers,
        data
      }).then(response => {
        this.$message({
    ...
    saveSorts() {
      ...
      axios({
        method: 'POST',
        url: baseApi(this.selectedStore.url_id, this.langId, 'specification/sorting'),
        headers: headers,
        params,
        data
      }).then(response => {
    ...
    remove(data) {
      ...
      axios({
        method: 'DELETE',
        url: baseApi(this.selectedStore.url_id, this.langId, 'specification/destroy/' + data.id),
        headers: headers
      }).then(response => {
    ...
    update(data) {
      ...
      axios({
        method: 'PUT',
        url: baseApi(this.selectedStore.url_id, this.langId, 'specification/update/' + data.id),
        headers: headers,
        data
      }).then(() => {
        this.getData()
