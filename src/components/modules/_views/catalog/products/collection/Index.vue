<template>
  <div>
    <div class="affix-wrapper flex-container flex-container--start flex-container--desktop">
      <div
        v-if="checkCustomPermission('catalog/collections', 'store')"
      ...
      <el-card v-loading="loading" shadow="never" class="flex-grow-1">
        <div slot="header" class="table-handler-flex">
          <h4 style="flex-grow: 1;">{{ $lang[langId].list }} {{ lang.collection }}</h4>
          <div v-if="showSaveSorts">
            <button-action-authenticated
              :permission="['catalog/collections', 'edit']"
              @click="saveSorts" 
...
export default {
  ...
  methods: {
    ...
    save(data) {
      ...
      axios({
        method: 'POST',
        url: baseApi(this.selectedStore.url_id, this.langId, 'collection/store'),
        headers: headers,
        data
      }).then(response => {
        this.getData()
    ...
    saveSorts() {
      ...
      axios({
        method: 'POST',
        url: baseApi(this.selectedStore.url_id, this.langId, 'collection/sorting'),
        headers: headers,
        params: {
    ...
    remove(data) {
      ...
      axios({
        method: 'DELETE',
        url: baseApi(this.selectedStore.url_id, this.langId, 'collection/destroy/' + data.id),
        headers: headers,
        params: {
          name: data.name
        }
    ...
    update(data) {
      ...
      axios({
        method: 'PUT',
        url: baseApi(this.selectedStore.url_id, this.langId, 'collection/update/' + data.id),
        headers: headers,
        data
      }).then(response => {
        this.getData()
