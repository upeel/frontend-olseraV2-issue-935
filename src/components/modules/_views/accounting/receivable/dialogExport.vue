<template>
  <el-dialog
    :title="'Export ' + this.$lang[langId].receivable"
    :visible.sync="show"
    @open="openExport"
    @close="closeExport"
    width="30%"
  ...
  methods: {
    ...
    getPDF() {
      ...
      if (this.$store.getters.webviewMode) {
        ...
        axios({
          url: baseApi(this.selectedStore.url_id, this.langId, 'account/receivable/pdf'),
          method: 'GET',
          responseType: 'blob',
          headers,
          params: params
          ...
          const fileName = this.$lang[this.langId].receivable + '-' + moment().format('yyyyMMDD') + '.pdf'
    ...
    getExcel() {
      ...
      if (this.$store.getters.webviewMode) {
        ...
        axios({
          method: 'GET',
          url: baseApi(this.selectedStore.url_id, this.langId, 'account/receivable/excel'),
          responseType: 'blob',
          headers,
          params
        }).then(response => {
            const fileName = this.$lang[this.langId].receivable + '-' + moment().format('yyyyMMDD') + '.csv'
    ...
    getFileMobile(type, data) {
      ...
      axios({
        ...
        this.loadingExport = false
        const file = new Blob([response.data])
        const fileName = this.$lang[this.langId].receivable + '-' + moment().format('yyyyMMDD') + '.' + type
        const url = window.URL.createObjectURL(file)
