  ...
  <div class="px-20">
    ...
    <div v-else-if="isSetup === 'true'">
      <el-card class="box-card" shadow="never">
        <div class="card-body">
          ...
          <div v-if="isLoading === false" style="margin-top: 10px;">
            <el-table
              ref="multipleTable"
              v-loading="isLoading"
              ...
              <el-table-column
                ...
                :label="lang.customer +' | '+ lang.description">
                <template slot-scope="scope">
                ...
                <template slot-scope="scope">
                  <el-dropdown trigger="click">
                    ...
                    <el-dropdown-menu style="background-color: #F2F2F2;">
                      <el-dropdown-item style="padding: 0px">
                        <el-button type="text" class="accounting-dropBtn" @click="detailReceivable(scope.row)">
                          <svg-icon icon-class="eye"></svg-icon>
                          {{ $lang[langId].detail }}
                        </el-button>
                      </el-dropdown-item>
                      <el-dropdown-item style="padding: 0px" v-if="scope.row.payment == 'Unpaid' || scope.row.payment == 'Belum dibayar' || scope.row.payment == 'Partial' || scope.row.payment == 'Parsial'">
    ...
    <el-dialog
      ...
      <el-form>
        ...
        <el-form-item :label="$lang[langId].receivable" :label-width="formLabelWidth">
          <el-radio-group v-model="filter.due_dates" style="border: groove; border-color: #409eff54; border-width: thin;">
            ...
            <el-radio-button label="false">
              {{ this.$lang[langId].all_receivable }}
            </el-radio-button>
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item class="visible-sm visible-xs" :label="lang.status" :label-width="formLabelWidth">
          ...
          <el-checkbox v-model="status.paid" :label="$lang[langId].paid_of" border size="small" style="margin-right: 0px;"></el-checkbox>
        </el-form-item>
      </el-form>
      ...
      <div slot="title" class="flex-container" style="text-align: center">
        ...
        <el-row :gutter="20"  style="width: 100%">
          ...
          <el-col :xs="12" :sm="8" :md="8" :lg="8" :xl="8" align="right">
            <div class="btn_accounting_dialog" v-if="temp.debit_account === 0 || temp.credit_account === 0">
      ...
      <span slot="footer" class="dialog-footer">
        <el-row :gutter="20" style="padding: 0 12px 0 12px;">
          ...
          <el-col :xs="24" :sm="16" :md="16" :lg="16" :xl="16" style="text-align: right;">
            ...
            <el-button :disabled="loadingDelete" type="danger" @click="handleRemoveJurnal(deleteData)">{{ lang.remove }}</el-button>
          </el-col>
        </el-row>
      </span>
    </el-dialog>
    ...
    return{
      ...
      filter: {
        ...
        amount: 0
          ...
          value: 'customer_name',
          label: this.$store.state.userStores.lang.customer_name
        },{
          value: 'int_amount',
          label: this.$lang[this.$store.state.userStores.langId].receivable_amount
        },{
          value: 'due_dates',
  ...
  methods: {
    ...
    toAddMultiJurnal () {
      this.$router.push({name: 'Add Multi Jurnal',
        params: {fromPage: 'Receivable List', lastParams: this.params, lastStatus: this.status, lastFilter: this.filter}
      })
    },
    ...
    getReceivable(){
      ...
      axios({
        method: 'GET',
        url: baseApi(this.selectedStore.url_id, this.langId, 'account/receivable'),
        headers: headers,
        params: this.params
      })
      .then(response => {
    ...
    detailReceivable (row) {
      let lastParams = this.params
      let lastStatus = this.status
      let lastFilter = this.filter
      this.$router.push({ name:'Receivable Detail', params: {id: row.id, data: row, lastParams: lastParams, lastStatus: lastStatus, lastFilter: lastFilter}})
    },
    ...
    showAddJurnal () {
      this.$router.push({ path: '/accounting/receivable/add-receivable'})
    },
    ...
    accept(row) {
      ...
      if(form === 1){
        ...
        axios({
          method: 'GET',
          url: baseApi(this.selectedStore.url_id, this.langId, 'account/receivable/' + pairId + '/' + id),
          headers: headers
        }).then(response => {
          this.temp = {
