    ...
    <div class="box sticky-top-has-submenu">
      ...
      <el-card class="box-card">
        <el-table
          ...
          stripe>
          <el-table-column
            :label="$lang[langId].store_name"
            width="250"
            prop="name">
            <template slot-scope="scope">
              <span class="word-break">{{ capitalize(scope.row.name)}}</span>
            </template>
          </el-table-column>
          <el-table-column
          ...
          <el-table-column
            ...
            width="400">
            <template slot-scope="scope">
              <div>{{scope.row.transaction_no}}</div>
              <div class="font-11">
...
export default {
  ...
  methods: {
    handleSearch (){
      ...
      axios({
        method: 'GET',
        url: baseApi(this.selectedStore.url_id, this.langId, 'account/gainbalance/allbranch/subdetail'),
        headers: headers,
        params: paramData
      }).then(response => {
    ...
    loadData(){
      ...
      axios({
        method: 'GET',
        url: baseApi(this.selectedStore.url_id, this.langId, 'account/gainbalance/allbranch/bystore'),
        headers: headers,
        params: paramsData
      }).then(response => {
        ...
        this.params.per_page = response.data.detail.per_page
    ...
    getFileMobile(type) {
      ...
      axios({
        url: baseApi(this.selectedStore.url_id, this.langId, 'account/gainbalance/allbranch/bystore/' + type),
        method: 'GET',
        responseType: 'blob',
        headers,
        params: params
        ...
        const apiURL = '/download/api/' + this.selectedStore.url_id + '/admin/v1/' + this.langId + '/account/gainbalance/allbranch/bystore/' + type
    ...
    getPDF() {
      ...
      if (this.$store.state.mobileReportView) {
        ...
        axios({
          url: baseApi(this.selectedStore.url_id, this.langId, 'account/gainbalance/allbranch/bystore/pdf'),
          method: 'GET',
          responseType: 'blob',
          headers,
          params: params 
          ...
          const file = new Blob([response.data])
          const fileName = this.data.account_name + '-' + this.$route.params.row.period + '.pdf'
          const url = window.URL.createObjectURL(file)
    ...
    getExcel() {
      ...
      if (this.$store.state.mobileReportView) {
        ...
        axios({
          method: 'GET',
          url: baseApi(this.selectedStore.url_id, this.langId, 'account/gainbalance/allbranch/detail/excel'),
          responseType: 'blob',
          headers,
          params
        }).then(response => {
            const fileName = this.data.account_name + '-' + this.$route.params.row.period + '.xlsx'
    ...
    backHandle() {
      ...
      this.$router.push({name: page, params: {period: period, typePeriod: typePeriod}})
    },
  },
  mounted() {
		this.loadStore()
