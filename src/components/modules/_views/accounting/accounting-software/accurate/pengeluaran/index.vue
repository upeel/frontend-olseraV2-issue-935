      ...
      <div class="title-jurnal">
        <h4>
          ...
          <div style="float: right; margin-top: -9px;">
            <el-button @click="handleCommandJurnal('mapOutlay')" type="primary" size="small">
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <el-select 
                  ...
                  :remote-method="getAccountOutlay"
                  style="width: 100%">
                  <el-option
    ...
    <el-dialog  
      ...
      <div align="left">
        ...
        <small style="font-weight: bolder;">
          ...
          {{ $lang[langId].outlay_to + ' Accurate ?'}}</small>
      </div>
    </el-dialog>
...
export default {
  ...
  methods:{
    ...
    getOutlay() {
      ...
      axios({
        method: 'GET',
        url: baseApi(this.selectedStore.url_id, this.langId, 'exaccurate/extrans'),
        headers:headers,
        params: this.params
      }).then(response => {
    ...
    getAccountOutlay(val){
      ...
      axios({
        method: 'GET',
        url: baseApi(this.selectedStore.url_id, this.langId, 'exaccurate/glaccount/allexpense'),
        headers: headers,
        params: paramsData
      }).then(response => {
    ...
    selectOutlay(value, rowKey){
      this.accountOutlay.map((item, index) => {
        if(item.no === value){
          this.dataOutlay[rowKey].debit_account_no = item.no
      ...
      axios({
        ...
        this.dataOutlay.forEach((val, no) => {
          if(val.id === parseInt(result.id)){
            this.dataOutlay[no].accurate_posted = 0
          }
        })
    ...
    exportOutlay(){
      this.loadingExport = true;
			if(this.dataOutlay.length === this.chosenOutlay.length){
				this.bulkExport()
			}else{
				this.commonExport()
			}
		...
		bulkExport(){
			...
			let data = []
      this.chosenOutlay.forEach((item, index) => {
        data.push({
          id: item.id,
          ...
          credit_account_name: item.credit_account_name
			...
			axios({
				...
				res.forEach(r => {
					let idx = this.dataOutlay.findIndex(x => x.id === parseInt(r.id));
					this.dataOutlay[idx].accurate_posted = r.accurate_posted
				})
        ...
        axios({
          ...
          this.dataOutlay.forEach((val, no) => {
            if(parseInt(res.id) === val.id){
              this.dataOutlay[no].accurate_posted = res.accurate_posted
            }
          })

          itemsProcessed++;
          if(itemsProcessed === this.chosenOutlay.length) {
            this.$refs.multipleTable.clearSelection();
            this.chosenOutlay = []
            this.getOutlay()
            this.$message({
