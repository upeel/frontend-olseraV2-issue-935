<template>
  <div>
    <div class="title-jurnal">
      <h4><strong>{{$lang[langId].transaction_type_debit}} ({{dataTransDebit.length}})</strong>
    ...
    <div class="title-jurnal">
      <h4><strong>{{$lang[langId].cash_credit}} ({{dataCashCredit.length}})</strong>
...
export default {
  ...
  methods: {
    ...
    loadDataDebit(){
      ...
      axios({
        method: 'GET',
        url: baseApi(this.selectedStore.url_id, this.langId, 'exaccurate/transtype/debit'),
        headers: headers
      }).then(response => {
        this.dataTransDebit = response.data.data
        this.loadingMapDebit = false
        let unmappedDebit = 0;
        this.dataTransDebit.map((row, idx) => {
          if(row.accurate_account_name === null){
            unmappedDebit += 1
          }
          this.dataTransDebit[idx].isModified = false
        })
    ...
    loadDataCredit(){
      ...
      axios({
        ...
        this.dataCashCredit = Array(response.data.data)
        this.loadingMapCredit = false 
        let unmappedCredit = 0;
        this.dataCashCredit.map((row, idx) => {
          if(row.accurate_account_name === null){
            unmappedCredit += 1
          }
          this.dataCashCredit[idx].isModified = false
        })
    ...
    selectAccount (type, val, row){
      let selected = {}
        if(type === 'Debit'){
          ...
          this.dataTransDebit.map((item, idx) => {
            if(item.id === row.id){
        ...
        if(type === 'Credit'){
          ...
          this.dataCashCredit.map((item, idx) => {
    ...
    mapAccountCredit(){
      ...
      axios({
        ...
        url: baseApi(this.selectedStore.url_id, this.langId, 'exaccurate/accountmap/cash'),
        headers: headers,
        data: this.editedAccountCredit[0]
      }).then(response => {
        this.loadingMapCredit = false
        this.result = response.data
