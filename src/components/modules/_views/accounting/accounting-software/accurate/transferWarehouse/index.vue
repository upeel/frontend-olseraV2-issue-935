      ...
      <div class="title-jurnal">
        <h4>
          ...
          <div style="float: right; margin-top: -9px;">
            <el-button @click="handleCommand('mapTransferProduct')" type="primary" size="small">
      ...
      <div class="jurnalExportHeader">
        <el-row :gutter="20">
          ...
          <el-col :md="12" :sm="12" :xs="24" align="right">
            <el-button
              :disabled="chosenStockOut.length === 0"
              :class="chosenStockOut.length !== 0 ? 'allowExport' : ''"
              size="small"
              type="info"
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <router-link :to="{path: '/inventory/stocksoutgoing/'+ scope.row.id,
                  query: {path:'accurate', pathName: 'Accurate', menu: 'transferWarehouse',
...
export default {
  ...
  data(){
    return{
      ...
      dialogUnpost: false,
			unpostData: {},
      itemPage: [{value: '15',label: '15 item'},{value: '25',label: '25 item' },
				{value: '50',label: '50 item'},{value: '100',label: '100 item'}],
			isLoading: false,
			labelDate: '',
      showFilter: false,
      params:{
  ...
  methods:{
    ...
    getStockOut() {
      ...
      axios({
        method: 'GET',
        url: baseApi(this.selectedStore.url_id, this.langId, 'exaccurate/item-transfer'),
        headers:headers,
        params: this.params
      }).then(response => {
    ...
    exportStockOut(){
      ...
      let headers = {
        Authorization: 'Bearer ' + this.token.access_token
			...
			let paramsData = {id: []}
      this.chosenStockOut.forEach((item, index, array) => {
				paramsData.id.push(item.id)
			})
			axios({
				method: 'POST',
				url: baseApi(this.selectedStore.url_id, this.langId, 'exaccurate/item-transfer'),
				...
				res.forEach((val, no) => {
					let idxPost = this.dataStockOut.findIndex( x => x.id === val.id )
					this.dataStockOut[idxPost].accurate_posted = val.accurate_posted
					this.dataStockOut[idxPost].accurate_transaction_no = val.accurate_transaction_no
				})
    ...
    handleUnpost(){
      ...
      axios({
        method: 'DELETE',
        url: baseApi(this.selectedStore.url_id, this.langId, 'exaccurate/item-transfer/' + id),
        headers: headers,
      }).then(response => {
        let result = response.data.data
				let idxPost = this.dataStockOut.findIndex( x => x.id === parseInt(result.id))
				this.dataStockOut[idxPost].accurate_transaction_no = ''
				this.dataStockOut[idxPost].accurate_posted = result.accurate_posted
