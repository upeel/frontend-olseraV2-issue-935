    ...
    <div style="margin-top: 24px">
      ...
      <el-table
        ...
        <el-table-column
          ...
          sortable>
          <template slot-scope="scope">
            <router-link v-if="scope.row.status === 'P'" :to="{path: '/purchase/openpurchase/'+ scope.row.id,
              query: {path:'accounting-third/accurate/purchase', pathName: 'Accurate Upload Purchase', lastParams: params, labelDate: labelDate} }">
              {{ scope.row.purchase_no }}
            </router-link>
            <router-link v-else :to="{path: '/purchase/completepurchase/'+ scope.row.id,
              query: {path:'accounting-third/accurate/purchase', pathName: 'Accurate Upload Purchase', lastParams: params, labelDate: labelDate} }">
              {{ scope.row.purchase_no }}
            </router-link>
          </template>
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-tag v-if="scope.row.accurate_payment === 1"
              type="success" size="mini"
              disable-transitions>{{$lang[langId].paid_off}}
            </el-tag>
    ...
    <el-dialog  
      ...
      <div align="left">
        ...
        <small style="font-weight: bolder;">
          ...
          {{ $lang[langId].purchase_to + ' Accurate ?'}}</small>
      </div>
    </el-dialog>
...
export default {
  ...
  methods:{
    ...
    getPurchase() {
      ...
      axios({
        method: 'GET',
        url: baseApi(this.selectedStore.url_id, this.langId, 'exaccurate/purchase'),
        headers:headers,
        params: this.params
      }).then(response => {
        ...
        this.$emit('invoicePostStatus', {
          ...
          finishMapingPurchase: response.data.finishProduct,
          isFecthSupplier: 0,
          isFecthProduct: 0
        })
    ...
    exportPurchase(){
      this.loadingExport = true;
			if(this.dataPurchase.length === this.chosenPurchase.length){
				this.bulkExport()
			}else{
				this.commonExport()
			}
		...
		bulkExport(){
			...
			this.chosenPurchase.forEach(function(v) {
				if(v.id){
					data.id.push(v.id)
				}else{
					data.id.push(0)
			...
			axios({
				method: 'POST',
				url: baseApi(this.selectedStore.url_id, this.langId, 'exaccurate/bulkpurchase'),
				headers: headers,
				data
			}).then(response => {
				let res = response.data.data
				res.forEach(r => {
					let idx = this.dataPurchase.findIndex(x => x.id === parseInt(r.id));
					this.dataPurchase[idx].accurate_posted = r.accurate_posted
					this.dataPurchase[idx].accurate_payment = r.accurate_payment
				})
				this.chosenPurchase = []
				this.$refs.multipleTable.clearSelection();
				this.$message({
        ...
        axios({
          ...
          this.dataPurchase.forEach((val, no) => {
            if(parseInt(item.id) === val.id){
              this.dataPurchase[no].accurate_posted = res.accurate_posted
							this.dataPurchase[no].accurate_payment = res.accurate_payment
            }
          })
    ...
    handleUnpost(){
      ...
      this.chosenUnpost.forEach((item, index, array) => {
        axios({
          method: 'DELETE',
          url: baseApi(this.selectedStore.url_id, this.langId, 'exaccurate/purchase/' + item.id),
          headers: headers,
        }).then(response => {
          let result = response.data.data
          this.dataPurchase.forEach((val, no) => {
            if(val.id === item.id){
              this.dataPurchase[no].accurate_posted = 0
							this.dataPurchase[no].accurate_payment = 0
            }
          })
          itemsProcessed++;
