      ...
      <div class="title-jurnal">
        <h4>
          ...
          <div style="float: right; margin-top: -9px;">
            <el-button @click="handleCommandJurnal('mapIncome')" :type="activeMapTrans ? 'info' : 'primary'" size="small">
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <el-select 
                  ...
                  @input="(value) => selectRevenue(value, scope.$index)"
                  style="width: 100%">
                  <el-option
                    v-for="item in accountRevenue"
                    :key="item.name"
      ...
      <div align="left">
        ...
        <div class="pt-10">
          <span v-if="!loadingExport">
            ...
            {{ $lang[langId].income_to + ' Jurnal.id ?'}}
          </span>
          <span v-else>{{$lang[langId].finish +' '+ multipleError.total_data + ' '+$lang[langId].of+' '+ this.chosenIncome.length +' '+ $lang[langId].revenue}}</span>
        </div>
      </div>
    </el-dialog>
...
import { invoiceRevenue, exportJurnalIncome, unpostMultiInvoice } from '@/api/accounting-3rd/jurnalRevenue';
...
export default {
  ...
  methods:{
    ...
    getIncome() {
      ...
      invoiceRevenue(this.params).then(response => {
    ...
    getAccountRevenue(){
      ...
      axios({
        ...
        this.accountRevenue = response.data.data
      }).catch(error => {
        if(typeof error.response.data.error.error === 'object'){
          let errorKey = Object.keys(error.response.data.error.error)[0];
          this.$notify({
    ...
    selectRevenue(value, rowKey){
      this.accountRevenue.map((item, index) => {
        if(item.number === value){
          this.dataIncome[rowKey].credit_account_no = item.number
      ...
      unpostMultiInvoice(id).then(response => {
        ...
        this.dataIncome.forEach((val, no) => {
          if(val.id === parseInt(result.id)){
            this.dataIncome[no].jurnal_posted = 0
          }
        })
    ...
    async exportIncome(){
      ...
      for (const item of this.chosenIncome) {
        ...
        let data = {
          ...
          credit_account_name: item.credit_account_name
				...
				await exportJurnalIncome(data).then(response => {
					let res = response.data.data
					this.dataIncome.forEach((val, no) => {
						if(parseInt(res.id) === val.id){
							this.dataIncome[no].jurnal_posted = res.jurnal_posted
						}
					})
