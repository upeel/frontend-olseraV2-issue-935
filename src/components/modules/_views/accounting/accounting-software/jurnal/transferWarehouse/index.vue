  ...
  <div class="px-20">
    ...
    <el-card class="box-card" shadow="never">
      ...
      <div class="title-jurnal">
        <h4>
          <strong>{{$lang[langId].upload + ' ' + $lang[langId].warehouse_transfer}}</strong>
          <div style="float: right; margin-top: -9px;">
            <el-button @click="handleCommandJurnal('mapTransferProduct')" :type="dataTransferProduct.length === 0 ? 'info' : 'primary'" size="small">
              {{$lang[langId].set_product_transfer}}
            </el-button>
      ...
      <div class="jurnalExportHeader tesss">
        <el-row :gutter="20">
          ...
          <el-col :md="12" :sm="12" :xs="24" align="right">
            <el-button
              :disabled="chosenStockOut.length === 0 || buttonPostStatus === false"
              :class="chosenStockOut.length !== 0 || buttonPostStatus ? 'allowExport' : ''"
              size="small"
              type="info"
          ...
          <el-table
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <router-link :to="{path: '/inventory/stocksoutgoing/'+ scope.row.id,
                  query: {path:'jurnalNew', pathName: 'Integrate Jurnal.ID', menu: 'transferWarehouse',
                  tab: 'transferWarehouse', lastParams: params, labelDate: labelDate} }">
                  {{ scope.row.trans_no }}
                </router-link>
              </template>
            </el-table-column>
            ...
            <el-table-column
              prop="transfer_to_store_name"
              :label="lang.transfer_to_store">
              <template slot-scope="scope">
                <span>{{ capitalize(scope.row.transfer_to_store_name) }}</span>
              </template>
            </el-table-column>
            <el-table-column
      ...
      <div slot="title" class="flex-container">
        <el-row :gutter="20" style="width: 100%">
          ...
          <el-col :span="12" align="right">
            ...
            <el-button :disabled="loadingExport" type="primary" @click="exportStockOut">
              <loading
                v-if="loadingExport"
                :active="true"
                color= '#1bb4e6'
      ...
      <div align="left">
        ...
        <div class="pt-10">
          <span v-if="!loadingExport">
            ...
            <span style="color: blue"> {{chosenStockOut.length}} </span>
            {{ lang.transactions + ' ?'}}
          </span>
          <span v-else>{{$lang[langId].finish +' '+ multipleError.total_data + ' '+$lang[langId].of+' '+ this.chosenStockOut.length +' '+ $lang[langId].warehouse_transfer}}</span>
        </div>
      </div>
    </el-dialog>
    ...
    <dialog-map-transfer
      :show="dialogMapTransferWarehouse"
      :dataProduct="dataTransferProduct"
      :fecthStatus="isfetchProduct"
      @close="closeMapTransferWarehouse"
    />
...
import DialogMapTransfer from '../DialogMapTransfer.vue';
import DialogTrowMultipleError from "@/components/DialogTrowMultipleError";
import { invoiceWarehouse, exportInvoiceWarehouse, unpostInvoiceWarehouse } from '@/api/accounting-3rd/jurnalWarehouse';
var moment = require('moment')

export default {
  name: 'ExportToJurnalStockOut',
  components: {
    CustomDateFilter,
    Loading,
    DialogMapTransfer,
  ...
  data(){
    return{
      ...
      dialogExport: false,
      dialogUnpost: false,
      dialogMapTransferWarehouse: false,
  ...
  methods:{
    ...
    handleCommandJurnal(command){
      if(command === 'mapTransferProduct'){
        this.dialogMapTransferWarehouse = true
    ...
    closeMapTransferWarehouse(){
      if(this.dataTransferProduct.length === 0){
        this.buttonPostStatus = true
      }
      this.dialogMapTransferWarehouse = false
    },
    ...
    getStockOut() {
      ...
      invoiceWarehouse(this.params).then(response => {
        ...
        this.dataTransferProduct = response.data.product.map(v => ({...v, isActive: false}))
    ...
    handleUnpost(){
      ...
      unpostInvoiceWarehouse(id).then(response => {
        let result = response.data.data
        this.dataStockOut.forEach((val, no) => {
          if(val.id === parseInt(result.id)){
            this.dataStockOut[no].jurnal_posted = result.jurnal_posted
          }
        })
        this.$message({
          type: 'success',
    ...
    async exportStockOut(){
      ...
      for (const item of this.chosenStockOut) {
        this.multipleError.total_data++
        let id = {id: item.id}
        await exportInvoiceWarehouse(id).then(response => {
          let res = response.data.data
          this.dataStockOut.forEach((val, no) => {
            if(parseInt(res.id) === val.id){
              this.dataStockOut[no].jurnal_posted = res.jurnal_posted
              this.dataStockOut[no].jurnal_transaction_no = res.jurnal_transaction_no
            }
          })
