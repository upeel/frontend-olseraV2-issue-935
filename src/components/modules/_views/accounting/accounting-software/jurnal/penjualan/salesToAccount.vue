  ...
  <div>
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="24" v-if="messageHidden === false">
            <el-tag @click="handleInfo" size="large" class="tag-jurnal-info">
              ...
              <p v-html="$lang[langId].export_sales_to_jurnal_account_info"></p>
            </el-tag>
          </el-col>
        </el-row>
      </div>
        ...
        <el-row :gutter="10">
          <el-col :md="12" :sm="12" :xs="12" align="left">
            ...
            <!-- <el-radio-group v-model="selectedMenu" @change="handleSelectMenu" size="small">
              ...
              <el-radio label="invoiceSales" border>{{$lang[langId].unpaid2}}</el-radio>
            </el-radio-group> -->
          </el-col>
      ...
      <div style="margin-top: 24px">
        ...
        <el-table
          ...
          <el-table-column
            ...
            sortable>
            <template slot-scope="scope">
              <router-link :to="{path: '/sales/completeorder/'+ scope.row.id,
                query: {path:'jurnalNew', pathName: 'Integrate Jurnal.ID', menu: 'general',
                tab: 'penjualan', lastParams: params, labelDate: labelDate} }">
                {{ scope.row.order_no }}
              </router-link>
            </template>
          </el-table-column>
    ...
    <el-dialog 
      ...
      <div class="text-center">
        ...
        <p style="font-weight: bold;">{{ unpostData.order_no }} - {{capitalize(unpostData.customer_name)}} - {{unpostData.ftotal_amount}}</p>
      </div>
      <span slot="footer" class="dialog-footer">
    ...
    <el-dialog  
      ...
      <div align="left">
        ...
        <small style="font-weight: bolder;">
          ...
          {{ $lang[langId].sales_to_general + ' Jurnal ?'}}</small>
      </div>
    </el-dialog>
...
export default {
  ...
  methods:{
    ...
    handleSelectMenu(){
      if(this.selectedMenu === 'invoiceSales'){
    ...
    getSales() {
      ...
      axios({
        method: 'GET',
        url: baseApi(this.selectedStore.url_id, this.langId, 'exjurnal/salestojurnal'),
        headers:headers,
        params: this.params
      }).then(response => {
    ...
    exportSales(){
      ...
      this.chosenSales.forEach((item, index) => {
        ...
        axios({
          method: 'POST',
          url: baseApi(this.selectedStore.url_id, this.langId, 'exjurnal/single/salestojurnal'),
          headers: headers,
          data: id
        }).then(response => {
          ...
          this.dataSales.forEach((val, no) => {
            if(parseInt(res.id) === val.id){
              this.dataSales[no].jurnal_posted = 1
              this.dataSales[no].jurnal_unposted = 0
            }
          })
    ...
    handleUnpost(){
      ...
      axios({
        method: 'DELETE',
        url: baseApi(this.selectedStore.url_id, this.langId, 'exjurnal/salestojurnal/' + id),
        headers: headers,
      }).then(response => {
        if(Object.keys(response.data).includes('data') === true){
          ...
          this.dataSales.forEach((val, no) => {
            if(val.id === result.id){
              this.dataSales[no].jurnal_posted = 0
              this.dataSales[no].jurnal_unposted = 0
            }
          })
          this.$message({
    ...
    handleInfo(){
      ...
      axios({
        method: 'POST',
        url: baseApi(this.selectedStore.url_id, this.langId, 'exjurnal/salestojurnal/message'),
        headers: headers,
      }).then(response => {
