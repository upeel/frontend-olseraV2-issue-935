      ...
      custom-class="dialog-card-jurnal">
        ...
        <el-tag size="large" class="tag-jurnal-info2">
          <el-row :gutter="10">
            ...
            <el-col :md="23" :sm="23" :xs="24">
              <p>{{$lang[langId].supplier_jurnal_info}}</p>
            </el-col>
          </el-row>
        </el-tag>
        ...
        <el-tabs v-model="activeTab" stretch>
          <el-tab-pane :label="lang.product" name="productMap">
            <product-map :allProduct="dataProduct" @toAllProduct="getAllProduct" :isFecth="fecthStatus"/>
          </el-tab-pane>
          <el-tab-pane :label="lang.supplier" name="supplierMap">
            <supplier-map :allSupplier="dataSupplier" :isFecth="fecthStatus"  @supplierList="dialogAllSupplier"/>
          </el-tab-pane>
        </el-tabs>
          ...
          <div v-show="showSetProduct" @keydown.esc="cardSetProduct" class="dialog-copy-from-other-store">
            ...
            <div class="sub-title mb-8">
              <label class="font-12 font-normal">
                {{rootLang.product_specific_terms}}
              </label>
              <el-tooltip class="item fl-right" effect="dark" placement="bottom"
                :content="rootLang.jurnal_text_product_addition">
              ...
              <el-form @submit.native.prevent>
                <el-form-item>
                  ...
                  <el-checkbox v-model="formSetting.end_space">
                    <span class="font-12">{{rootLang.end_word_space}}</span>
                  </el-checkbox>
        ...
        <el-row :gutter="20"  style="width: 100%">
          <el-col :span="14">
            <h4 class="dialog-title" align="left">
              <strong>{{ $lang[langId].supplier_list }}</strong>
            </h4>
          </el-col>
          <el-col :span="10" align="right">
            <div class="btn_save_dialog">
										...
										<el-col :span="24">
											<div class="biodata mb-8">
												<el-button :disabled="loadingAllPost" type="primary" @click="allPostSupplier" size="small">
													...
													<span v-else>{{$lang[langId].post_all_supplier_to}} Jurnal.id</span>
												</el-button><br>
												<p class="font-12" v-html="$lang[langId].jurnal_all_post_supplier_text"></p>
											</div>
											<el-divider></el-divider>
											<div class="biodata mb-8">
												<el-button :disabled="loadingSync" type="info" @click="syncUnpostSupplier" size="small">
													<span v-if="loadingSync">
														<loading
															:active="true"
															color= '#1bb4e6'
														...
														<span v-if="totalSyncSupplier !== 0">{{$lang[langId].page + ' ' + currentSyncSupplier + ' '+$lang[langId].of + ' ' + totalSyncSupplier}}</span>
													</span>
													<span v-else>{{$lang[langId].sync_supplier}} Jurnal.id</span>
												</el-button><br>
												<p class="font-12" v-html="$lang[langId].jurnal_sync_supplier_text"></p>
											</div>
										</el-col>
									</el-row>
								</div>
              ...
              <el-button type="info" @click="dialogAllSupplier">{{ lang.close }}</el-button>
            </div>
          </el-col>
        </el-row>
      </div>

      <supplier-mapped v-if="showAllSupplier" :isFecth="fecthStatus"/>
    </el-dialog>
  </div>
</template>
...
import SupplierMap from 'components/modules/_views/accounting/accounting-software/jurnal/mapping/mapSupplier.vue';
...
import { syncProduct, syncSupplier  } from '@/api/accounting-3rd/jurnalid'
...
export default {
  ...
  data() {
    return {
      ...
      currentSync: 0,
      totalSync: 0,
			paramsSupplier: {start_date: '', end_date: '', page: 1},
      currentSyncSupplier: 0,
      totalSyncSupplier: 0,
  ...
  methods:{
    ...
    cardSetProduct(){
      ...
      this.loadingSet = false
		...
		syncUnpostSupplier(){
      this.loadingSync = true
      syncSupplier(this.paramsProduct).then(response => {
        let message = 'Success ' + this.paramsSupplier.page + ' ' + this.rootLang.of + ' ' + response.data.data.total_pages
        if (this.paramsSupplier.page < response.data.data.total_pages) {
          this.currentSyncSupplier = response.data.data.currenct_page + 1
          this.totalSyncSupplier = response.data.data.total_pages
          this.paramsSupplier.page = response.data.data.currenct_page + 1
          this.$message({
            type: 'success',
            message: message
          })
          this.syncUnpostSupplier()
        } else {
          this.$message({
            type: 'success',
            message: message
      ...
      this.loadingSet = true
		...
		handleAllPost(){
			...
			axios({
				...
				if (resp.finish === 0) {
					this.handleAllPost()
					this.alProductPending = resp.module + ' - ' + this.rootLang.waiting + ' ' + resp.pending + ' ' + this.lang.item
				}else{
		...
		allPostSupplier(){
			...
			axios({
				method: 'POST',
				url: baseApi(this.selectedStore.url_id, this.langId, 'exjurnal/vendorpost'),
				headers: headers,
			}).then(response => {
				let resp = response.data
		...
		dialogAllSupplier() {
			let show = this.showAllSupplier
      this.showAllSupplier = show ? false : true
    },
  }
}
</script>
