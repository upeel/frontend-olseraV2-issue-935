    ...
    <div v-if="isLoading === false">
      <el-table
        ...
        <el-table-column
          ...
          :label="lang.address">
          <template slot-scope="scope">
            <span>{{capitalize(scope.row.address)}}</span>
          </template>
        </el-table-column>
        <el-table-column
            prop="code"
            ...
            >
            <template slot-scope="scope">
              <el-tag v-if="scope.row.id === parseInt(selectedWH.jurnal_id)"
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div style="float: right">
              <el-popconfirm v-if="scope.row.id !== parseInt(selectedWH.jurnal_id)"
...
export default {
  ...
  methods: {
    loadData(){
      ...
      axios({
        method: 'GET',
        url: baseApi(this.selectedStore.url_id, this.langId, 'exjurnal/warehouse'),
        headers: headers,
        params: this.params
      }).then(response => {
        ...
        if(response.data.selected){
          ...
          this.selectedWH = {id: 0, jurnal_id: 0}
        }
    ...
    useWarehouse(val){
      ...
      axios({
        ...
        if (response.data.status === 200){
          this.selectedWH = response.data.data
          this.$message({
            message: 'Success',
            type: 'success'
          })
    ...
    deleteWarehouse(val){
      ...
      axios({
        method: 'DELETE',
        url: baseApi(this.selectedStore.url_id, this.langId, 'exjurnal/warehouse/'+this.selectedWH.id),
        headers: headers,
        data: paramData
      }).then(response => {
        if (response.data.status === 200){
