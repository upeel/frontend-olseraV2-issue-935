<template>
  <div>
    <div class="title-jurnal">
      <h4><strong>{{$lang[langId].account_jurnal}} ({{dataJurnalAccount.length}})</strong>
...
export default {
  ...
  methods: {
    ...
    loadData(){
      ...
      axios({
        method: 'GET',
        url: baseApi(this.selectedStore.url_id, this.langId, 'exjurnal/accountmap'),
        headers: headers,
        params: this.params
      }).then(response => {
        this.dataJurnalAccount = response.data.data
        let unmapped = 0;
        this.dataJurnalAccount.map((row, idx) => {
          if(row.jurnal_account_name === null){
            unmapped += 1
          }
          this.dataJurnalAccount[idx].isModified = false
        })
    ...
    selectAccount (val, row){
      ...
      this.dataJurnalAccount.map((item, idx) => {
