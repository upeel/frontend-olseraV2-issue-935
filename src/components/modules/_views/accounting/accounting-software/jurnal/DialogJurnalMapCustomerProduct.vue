      ...
      custom-class="dialog-card-jurnal">
        ...
        <el-tag size="large" class="tag-jurnal-info2">
          <el-row :gutter="10">
            ...
            <el-col :md="23" :sm="23" :xs="24">
              <p>{{$lang[langId].customer_jurnal_info}}</p>
            </el-col>
          </el-row>
        </el-tag>
        ...
        <el-tabs v-model="activeTab" stretch>
          ...
          <el-tab-pane :label="lang.customer" name="customerMap">
            <customer-map :allCustomer="dataCustomer" :isFecth="fecthStatus" @customerList="dialogAllCustomer"/>
          </el-tab-pane>
        </el-tabs>
        ...
        <el-row :gutter="20"  style="width: 100%">
          <el-col :span="14">
            <h4 class="dialog-title" align="left">
              <strong>{{ $lang[langId].customer_list }}</strong>
            </h4>
          </el-col>
          <el-col :span="10" align="right">
            <div class="btn_save_dialog">
										...
										<el-col :span="24">
											<div class="biodata mb-8">
												<el-button :disabled="loadingAllPost" type="primary" @click="allPostCustomer" size="small">
													...
													<span v-else>{{$lang[langId].post_all_customer_to}} Jurnal.id</span>
												</el-button><br>
												<p class="font-12" v-html="$lang[langId].jurnal_all_post_customer_text"></p>
											</div>
											<el-divider></el-divider>
											<div class="biodata mb-8">
												<el-button :disabled="loadingSync" type="info" @click="SyncUnpostCustomer" size="small">
													<span v-if="loadingSync">
														<loading
															:active="true"
															color= '#1bb4e6'
														...
														<span v-if="totalSyncCustomer !== 0">{{$lang[langId].page + ' ' + currentSyncCustomer + ' '+$lang[langId].of + ' ' + totalSyncCustomer}}</span>
													</span>
													<span v-else>{{$lang[langId].sync_customer}} Jurnal.id</span>
												</el-button><br>
												<p class="font-12" v-html="$lang[langId].jurnal_sync_customer_text"></p>
											</div>
										</el-col>
									</el-row>
								</div>
              ...
              <el-button type="info" @click="dialogAllCustomer">{{ lang.close }}</el-button>
            </div>
          </el-col>
        </el-row>
      </div>

      <customer-mapped v-if="showAllCustomer" :isFecth="fecthStatus"/>
    </el-dialog>
  </div>
</template>
...
import CustomerMap from 'components/modules/_views/accounting/accounting-software/jurnal/mapping/mapCustomer.vue';
...
import { syncProduct, syncCustomer } from '@/api/accounting-3rd/jurnalid'
...
export default {
  ...
  data() {
    return {
      ...
      currentSync: 0,
      totalSync: 0,
      paramsCustomer: {start_date: '', end_date: '', page: 1},
      currentSyncCustomer: 0,
      totalSyncCustomer: 0,
  ...
  methods:{
    ...
    SyncUnpostCustomer(){
      this.loadingSync = true
      syncCustomer(this.paramsCustomer).then(response => {
        let message = 'Success ' + this.paramsCustomer.page + ' ' + this.rootLang.of + ' ' + response.data.data.total_pages
        if (this.paramsCustomer.page < response.data.data.total_pages) {
          this.currentSyncCustomer = response.data.data.currenct_page + 1
          this.totalSyncCustomer = response.data.data.total_pages
          this.paramsCustomer.page = response.data.data.currenct_page + 1
          this.$message({
            type: 'success',
            message: message
          })
          this.SyncUnpostCustomer()
        } else {
          this.$message({
            type: 'success',
            message: message
    ...
    saveSettingProduct(){
      this.loadingSet = true
		...
		allPostCustomer(){
			...
			axios({
				method: 'POST',
				url: baseApi(this.selectedStore.url_id, this.langId, 'exjurnal/customerpost'),
				headers: headers,
			}).then(response => {
				let resp = response.data
		...
		dialogAllCustomer() {
			let show = this.showAllCustomer
      this.showAllCustomer = show ? false : true
    },
  }
}
</script>
