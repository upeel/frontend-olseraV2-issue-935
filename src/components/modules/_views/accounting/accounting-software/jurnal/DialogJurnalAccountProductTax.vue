      ...
      custom-class="dialog-card">
        ...
        <el-row :gutter="10" class="pb-12">
          <el-col :xl="18" :lg="19" :md="18" :sm="18" :xs="24" align="left">
            <el-button @click="secletAccountTab('productAccount')" :type="activeTab === 'productAccount' ? 'primary' : 'info'" round plain size="small">{{$lang[langId].account_product}}</el-button>
            <el-button @click="secletAccountTab('taxList')" :type="activeTab === 'taxList' ? 'primary' : 'info'" round plain size="small">{{$lang[langId].tax_list}}</el-button>
            ...
            <el-button @click="secletAccountTab('warehouse')" :type="activeTab === 'warehouse' ? 'primary' : 'info'" round plain size="small">{{$lang[langId].warehouse}}</el-button>
          </el-col>
        ...
        <product-account v-if="activeTab === 'productAccount' && typeProduct === 'general'" :load="loadData" @changeProduct="changeProduct"/>
        <product-advanced v-if="activeTab === 'productAccount' && typeProduct === 'advanced'" :load="loadData" @changeProduct="changeProduct"/>
        <tax-list v-else-if="activeTab === 'taxList'" :load="loadData"/>
        <payment-method v-else-if="activeTab === 'paymentMethod'" :load="loadData"/>
        <other-account v-else-if="activeTab === 'otherAccount'" :load="loadData"/>
...
<script>
import ProductAccount from 'components/modules/_views/accounting/accounting-software/jurnal/mapping/productAccount.vue';
import ProductAdvanced from 'components/modules/_views/accounting/accounting-software/jurnal/mapping/productAdvanced.vue';
import TaxList from 'components/modules/_views/accounting/accounting-software/jurnal/mapping/taxList.vue';
import PaymentMethod from 'components/modules/_views/accounting/accounting-software/jurnal/mapping/paymentMethod.vue';
import OtherAccount from 'components/modules/_views/accounting/accounting-software/jurnal/mapping/invoiceOtherAccount.vue';
import Warehouse from 'components/modules/_views/accounting/accounting-software/jurnal/mapping/mapWarehouse';
...
export default {
  ...
  methods:{
    ...
    copyFromStore(){
      ...
      axios({
        method: 'POST',
        url: baseApi(this.selectedStore.url_id, this.langId, 'exjurnal/copymap/account'),
        headers: headers,
        params: data
      }).then(response => {
    ...
    changeProduct(){
      let type = this.typeProduct
      this.typeProduct = type === 'general' ? 'advanced' : 'general'
    },
