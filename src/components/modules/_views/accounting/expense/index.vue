        ...
        <div v-else-if="isSetup === 'true'" >
          <el-card class="box-card" shadow="never">
            <div class="row">
              ...
              <div class="visible-lg visible-md col-lg-6 col-md-6 text-center" >
                  <single-month-picker
            ...
            <div v-if="newJurnal.length !== 0" style="margin-top: 24px">
                <div class="box collapsed-box" style="width: 100%; background: #F5F5F5">
                  ...
                  <div class="box-body create-product" style="background: #FFFFFF; padding: 0px; display: none;">
                    <el-table
                      ref="multipleTable"
                  ...
                  <el-table-column
                    ...
                    <template slot-scope="scope">
                      <div v-if="scope.row.famount !== null">
                        <router-link :to="{ name:'Expense - Detail', params: {row: {year: scope.row.year, month: scope.row.month, account_no: scope.row.account_no, account_name: scope.row.account_name},
                          lastParams: params, lastFilter: filter, monthLabel: selectedMonth}}">
                          <el-popover
                            placement="top-start"
...
export default {
  ...
  methods: {
    ...
    updateParamTrx(event){
      let exist = 'transaction' in this.params
      if(event === false && exist === true){
        delete this.params.transaction;
      }

      if(event === true && exist === false){
        this.params.transaction = 'true';
      }
    },
    ...
    toAddMultiJurnal () {
      this.$router.push({name: 'Add Multi Jurnal',
        params: {fromPage: 'Expense', lastParams: this.params, lastFilter: this.filter, monthLabel: this.selectedMonth}
      })
    },
    ...
    getExpense(){
      ...
      axios({
        method: 'GET',
        url: baseApi(this.selectedStore.url_id, this.langId, 'account/expenses'),
        headers: headers,
        params: this.params
      }).then(response => {
