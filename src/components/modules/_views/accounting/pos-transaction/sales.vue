      ...
      <el-card class="box-card" shadow="never">
        <div class="row">
          ...
          <div class="col-xs-2 col-sm-4 col-lg-4 col-md-4 text-right">
            ...
            <el-button type="primary" size="small" :disabled="chosenSales.length === 0" @click="showValidate('post')">
              Post
            </el-button>
          </div>
        </div>
            ...
            <el-table
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  <span v-if="scope.row.order_date !== null">{{ scope.row.forder_date }}</span>
                  <span v-else>-</span>
                </template>
              </el-table-column>
              <el-table-column
                ...
                <template slot-scope="scope">
                  <router-link v-if="scope.row.status === 'Z'" :to="{ path: '/sales/completeorder/' + scope.row.id, params: {row: {inPage: 'Sales POS Transaction', data: scope.row, lastParams: params, selectedMonth: labelDate}}}">
                    {{ scope.row.order_no }}
                  </router-link>
                  <router-link v-else :to="{ path: '/sales/openorder/' + scope.row.id, params: {row: {inPage: 'Sales POS Transaction', data: scope.row, lastParams: params, selectedMonth: labelDate}}}">
                    {{ scope.row.order_no }}
                  </router-link>
                </template>
              </el-table-column>
    ...
    <el-dialog :visible.sync="dialogValidatePost" :show-close="false"
      ...
      :close-on-click-modal="false">
        ...
        <div v-if="!loadingPost" class="font-24 semi-bold text-center">
            <span>Post {{ this.chosenSales.length +' '+ $lang[langId].sales }} ?</span>
          <div style="margin-top: 20px; display: flex;">
            <el-button type="text" @click="postSales" class="color-success font-24" style="width: 50%;">
              {{ lang.yes }}
            </el-button>
        ...
        <div v-else class="font-24 semi-bold text-center">
          <span>{{$lang[langId].finish +' '+ multipleError.total_data + ' '+$lang[langId].of+' '+ this.chosenSales.length +' '+ $lang[langId].sales}}</span>
    ...
    <el-dialog :visible.sync="dialogValidateUnpost" :show-close="false"
      ...
      :close-on-click-modal="false">
        ...
        <div v-if="!loadingUnpost" class="font-24 semi-bold text-center">
            <span>Unpost {{ this.chosenUnpost.length +' '+ $lang[langId].sales }} ?</span>
          <div style="margin-top: 20px; display: flex;">
        ...
        <div v-else class="font-24 semi-bold text-center">
          <span>{{$lang[langId].finish +' '+ multipleError.total_data + ' '+$lang[langId].of+' '+ this.chosenUnpost.length +' '+ $lang[langId].sales}}</span>
          <div style="margin-top: 20px">
            <loading 
              align="center"
              :active="true"
...
import { salesList, salesPost, salesUnpost } from '@/api/accounting/pos-transaction';
...
export default {
  ...
  methods: {
    ...
    getPOSTrx(){
      ...
      salesList(this.params).then(response => {
    ...
    async postSales(){
      ...
      let sorted = this.chosenSales.sort(function(a, b) { 
        return a.id - b.id;
      });
      ...
      for (const chosen of sorted) {
        ...
        await salesPost(data).then(response => {
      ...
      for (const chosen of sorted) {
        this.multipleError.total_data++;
        const data = { id: chosen.id }
        await salesUnpost(chosen.id).then(response => {
          this.dataPOS.forEach(item => {
            if(item.id === chosen.id){
              item.posted = 0
