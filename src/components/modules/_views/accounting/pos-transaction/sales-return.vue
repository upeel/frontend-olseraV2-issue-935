<template>
  <div class="px-20">
    ...
    <div v-else-if="isSetup === 'true'" >
      <el-card class="box-card" shadow="never">
        <div class="row">
          <div class="col-xs-2 col-lg-8 col-md-8 col-sm-8 text-left" style="display: -webkit-inline-box;">
            <el-button
              type="info"
          ...
          <div class="col-xs-2 col-sm-4 col-lg-4 col-md-4 text-right">
            ...
            <el-button type="primary" size="small" :disabled="chosenSalesReturn.length === 0" @click="showValidate('post')">
              Post
            </el-button>
          </div>
        </div>
            ...
            <el-table
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  <span v-if="scope.row.return_date !== null">{{ scope.row.freturn_date }}</span>
                  <span v-else>-</span>
                </template>
              </el-table-column>
              <el-table-column
                ...
                <template slot-scope="scope">
                  <router-link :to="{ path: '/sales/returnorder/' + scope.row.id, params: {row: {inPage: 'Sales Return POS Transaction', data: scope.row, lastParams: params, selectedMonth: labelDate}}}">
                    {{ scope.row.return_no }}
                  </router-link>
                </template>
              </el-table-column>
              ...
              <el-table-column
                ...
                :label="lang.customer">
                <template slot-scope="scope">
                  <div v-if="scope.row.customer_name">{{ capitalize(scope.row.customer_name) }}</div>
                  <div class="font-12" v-if="scope.row.customer_email">{{ scope.row.customer_email }}</div>
                </template>
              </el-table-column>
              <el-table-column
                prop="total_amount"
      ...
      <el-form>
        <el-form-item :label="lang.search" :label-width="formLabelWidth">
          <el-input
            ...
            :placeholder="lang.return_no+'/'+lang.customer_name"
            clearable
    ...
    <el-dialog :visible.sync="dialogValidatePost" :show-close="false"
      ...
      :close-on-click-modal="false">
        ...
        <div v-if="!loadingPost" class="font-24 semi-bold text-center">
            <span>Post {{ this.chosenSalesReturn.length +' '+ $lang[langId].salesreturn }} ?</span>
          <div style="margin-top: 20px; display: flex;">
            <el-button type="text" @click="postSalesReturn" class="color-success font-24" style="width: 50%;">
              {{ lang.yes }}
            </el-button>
        ...
        <div v-else class="font-24 semi-bold text-center">
          <span>{{$lang[langId].finish +' '+ multipleError.total_data + ' '+$lang[langId].of+' '+ this.chosenSalesReturn.length +' '+ $lang[langId].salesreturn}}</span>
    ...
    <el-dialog :visible.sync="dialogValidateUnpost" :show-close="false"
      ...
      :close-on-click-modal="false">
        ...
        <div v-if="!loadingUnpost" class="font-24 semi-bold text-center">
            <span>Unpost {{ this.chosenUnpost.length +' '+ $lang[langId].salesreturn }} ?</span>
          <div style="margin-top: 20px; display: flex;">
        ...
        <div v-else class="font-24 semi-bold text-center">
          <span>{{$lang[langId].finish +' '+ multipleError.total_data + ' '+$lang[langId].of+' '+ this.chosenUnpost.length +' '+ $lang[langId].salesreturn}}</span>
          <div style="margin-top: 20px">
            <loading 
              align="center"
              :active="true"
...
import { salesReturnList, salesReturnPost, salesReturnUnpost } from '@/api/accounting/pos-transaction';
...
export default {
  name: 'SalesReturnPOSTransaction',
  ...
  methods: {
    ...
    getPOSTrx(){
      ...
      salesReturnList(this.params).then(response => {
    ...
    async postSalesReturn(){
      ...
      this.multipleError.title= 'Post ' + this.$lang[this.langId].salesreturn

      for (const chosen of this.chosenSalesReturn) {
        this.multipleError.total_data++;
        let data = {
          id: chosen.id
        }

        await salesReturnPost(data).then(response => {
          this.dataPOS.forEach(item => {
            if(item.id === chosen.id){
              item.posted = 1
            }
          });
          this.multipleError.total_succed++
          if(this.multipleError.total_data === this.chosenSalesReturn.length) {
            clearSelections()
          ...
          this.multipleError.errorList.push({ name: chosen.return_no, desc: error.string })
          if(this.multipleError.total_data === this.chosenSalesReturn.length) {
            clearSelections()
            this.multipleErrorShow = true
          }
      ...
      this.multipleError.title= 'Unpost ' + this.$lang[this.langId].salesreturn

      for (const chosen of sorted) {
        this.multipleError.total_data++;
        let deleteId = chosen.id
        await salesReturnUnpost(deleteId).then(response => {
          this.dataPOS.forEach(item => {
            if(item.id === chosen.id){
              item.posted = 0
            }
