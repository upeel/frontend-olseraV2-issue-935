    ...
    <div v-else-if="isSetup === 'true'" >
      <el-card class="box-card" shadow="never">
        <div slot="header" class="table-handler-flex">
          <h4 style="flex-grow: 1;">{{rootLang.income_data}}</h4>
        ...
        <div class="card-body">
          <div class="row">
            ...
            <div class="col-xs-12 col-sm-4 col-lg-4 col-md-4 text-right">
              ...
              <el-button type="primary" size="small" :disabled="chosenIncome.length === 0" @click="showValidate('post')">
                Post
              </el-button>
            </div>
          </div>
          <div v-if="isLoading === false" class="mt-4">
            <el-table
                    ...
                    <table class="acounting-nested-table" style="width: 100%;">
                      <thead>
                        <tr class="nested-table-name-row">
                          <th class="px-20" style="text-align: left;">{{$lang[langId].transaction_types}}</th>
                          <th class="px-20" style="text-align: left;">{{lang.notes}}</th>
                        </tr>
                      </thead>
                      <tbody>
                        <tr class="nested-table-summary-list">
                          <td class="px-20">{{scope.row.trans_type_name}}</td>
                          <td class="px-20">{{scope.row.notes}}</td>
                        </tr>
                      </tbody>
                    </table>
                    <el-button slot="reference" type="text">{{scope.row.trans_no}}</el-button>
                  </el-popover> -->
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
									<div>{{scope.row.trans_date !== null ? scope.row.trans_date : '-'}}</div>
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  <el-select 
                    ...
                    style="width: 100%">
                    <el-option
                      v-for="(item, keyDebit) in dataCashBank"
                      :key="keyDebit"
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  <el-select 
                    ...
                    style="width: 100%">
                    <el-option
                      v-for="(item, keyCredit) in dataRevenue"
                      :key="keyCredit"
                      :label="item.account_no+' '+item.account_name"
                      :value="item.id">
                    </el-option>
                  ...
                  <!-- <div>
                    {{scope.row.credit_id ? scope.row.credit_account +' '+ scope.row.credit_account_name: '-'}}
                  </div> -->
                </template>
              </el-table-column>
    ...
    <el-dialog :visible.sync="dialogValidatePost" :show-close="false"
      ...
      :close-on-click-modal="false">
        ...
        <div v-if="!loadingPost" class="font-24 semi-bold text-center">
            <span>Post {{ this.chosenIncome.length +' '+ $lang[langId].revenue }} ?</span>
        ...
        <div v-else class="font-24 semi-bold text-center">
          <span>{{$lang[langId].finish +' '+ multipleError.total_data + ' '+$lang[langId].of+' '+ this.chosenIncome.length +' '+ $lang[langId].revenue}}</span>
    ...
    <el-dialog :visible.sync="dialogValidateUnpost" :show-close="false"
      ...
      :close-on-click-modal="false">
        ...
        <div v-if="!loadingUnpost" class="font-24 semi-bold text-center">
            <span>Unpost {{ this.chosenUnpost.length +' '+ $lang[langId].revenue }} ?</span>
          <div style="margin-top: 20px; display: flex;">
        ...
        <div v-else class="font-24 semi-bold text-center">
          <span>{{$lang[langId].finish +' '+ multipleError.total_data + ' '+$lang[langId].of+' '+ this.chosenUnpost.length +' '+ $lang[langId].revenue}}</span>
          <div style="margin-top: 20px">
            <loading 
              align="center"
              :active="true"
...
import { incomeList, incomePost, inexUnpost, cashBankOnly, revenueOnly} from '@/api/accounting/pos-transaction';
import DialogIncomeTransType from './transType';
...
export default {
  name: 'IncomePOSTransaction',
  mixins: [mixinAccounting, basicComputedMixin],
  components: {
    Loading,
    CustomDateFilter,
		dialogSetup,
		DialogIncomeTransType,
  ...
  methods: {
    ...
    getPOSIncome(){
      ...
      incomeList(this.params).then(response => {
    ...
    async saveHandle(index){
      ...
      let sorted = this.chosenIncome.sort(function(a, b) { 
        return a.id - b.id;
      });
      ...
      this.multipleError.title= 'Post ' + this.$lang[this.langId].revenue
      ...
      for (const chosen of sorted) {
        ...
        await incomePost(edited).then(response => {
          ...
          if(this.multipleError.total_data === this.chosenIncome.length) {
            clearSelections()
      ...
      this.multipleError.title= 'Unpost ' + this.$lang[this.langId].revenue
