    ...
    <section class="content">
      <el-card class="box-card">
        <div slot="header" class="table-handler-flex">
          <h4 style="flex-grow: 1;">{{ lang.banner }}</h4>
          <button-action-authenticated
            :permission="['settings/mobilebanner', 'store']"
            @click="add"
            size="small"
            type="success"
            icon="el-icon-plus"
        ...
        <div class="card-body">
          ...
          <div class="load-more" v-if="moreLink" v-loading="loadingItems">
            <el-button :disabled="disabledButton" @click="loadMore" class="btn-block">
...
const apiEndpoint = 'mobilebanner/'
...
export default {
  ...
  methods: {
    ...
    add() {
      this.labelDialog = this.lang.add + ' ' + this.lang.banner
      this.showForm = true
    },
    edit(item) {
      this.labelDialog = this.lang.change_banner
      let id = item.id
    ...
    save(val) {
      let saveData = {
        title: val.title,
        link_type_id: val.link_type_id,
        file_name: val.photo_name,
        has_link: val.has_link,
