<template>
  <div>
    <div>
      <el-form label-width="40%">
        ...
        <el-form-item v-if="data.link_type_id === 'PGR'">
          ...
          >
            <i
              v-if="data.product_group_id"
              slot="suffix"
              class="el-icon-circle-close-outline"
              @click="clearSearch('product', data)"
            />
          </el-autocomplete>
        </el-form-item>
        ...
        <el-form-item v-if="data.link_type_id === 'ATT'">
          ...
          <el-autocomplete v-model="articleValue" :fetch-suggestions="searchArticle" :debounce="debounce" :placeholder="lang.search+' '+lang.article" @select="handleSelect('article', $event)">
        ...
        <el-form-item :label="lang.link_title">
          <el-input v-model="data.title"></el-input>
        </el-form-item>
        <el-form-item v-if="data.id">
          <button-action-authenticated :permission="['settings/mobilenav', 'destroy']" @click="remove" type="danger" plain icon="el-icon-delete" style="width: 100%; background: #fff;">
            {{ $lang[langId].delete }}
    ...
    <div style="text-align: center;">
      ...
      <button-action-authenticated :permission="['settings/mobilenav', 'edit']" :disabled="disabledSave" @click="save" type="success" icon="el-icon-check">
        {{ lang.save }}
      </button-action-authenticated>
    </div>
  </div>
...
export default {
  ...
  watch: {
    data: {
      handler(data) {
        console.log(data)
        if (data.link_type_id === 'PGR') {
          if (data.title && data.product_group_id) {
