      ...
      <div slot="title" class="flex-container" style="text-align: center">
        <el-row :gutter="20"  style="width: 100%">
          ...
          <el-col :xs="21" :sm="22" :md="23" :lg="23" :xl="23" align="center">
            <h4 class="dialog-title font-24">BFI Finance</h4>
          </el-col>
        </el-row>
      </div>
        ...
        <div class="col-lg-8" style="float: none; margin: auto">
          <div class="like-table-wrapper mb-16" style="box-shadow: 0px 2px 2px 2px #0503031f;">
            <div class="like-table-wrapper--item pointer">
              ...
              <el-button class="color-bfi--bg color-white" @click="submitAgain" :loading="loadingCheck">{{rootLang.submit_again}} <i class="el-icon-arrow-right"></i></el-button>
            </div>
          </div>

          <funding-list @bfiId="IdBFI"/>
        </div>
      </div>
    </el-dialog>
  </div>
      ...
      bfiStore().then(async response => {
        ...
        if(lastSubmission.submission_req[0].submission_status === 'Approved' || lastSubmission.submission_req[0].submission_status === 'Rejected'){
          this.$router.push({
            ...
            query: {bfi_id: lastSubmission.id}
          })
        }else{
          this.$message({
            type: 'error',
