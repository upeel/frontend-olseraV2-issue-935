    ...
    <div style="margin-top: 24px">
      <div v-if="isLoading === false" style="border: 1px solid #f5f5f5; border-radius: 3px;">
        <el-table
          ...
          <el-table-column
            :label="rootLang.plafond"
            prop="tenor">
            <template slot-scope="scope">
              <span>{{ scope.row.tenor }} {{rootLang.months}}</span>
            </template>
          </el-table-column>
          <el-table-column
...
export default {
  ...
  methods: {
    getFunding(){
      ...
      bfiStore(params).then(async response => {
        this.isLoading = false
        this.dataBFI = response.data.data.submission_req
        this.$emit('bfiId', response.data.data.id)
      }).catch(error => {
        this.isLoading = false
        this.$message({
          type: 'error',
