          ...
          <div class="like-table-wrapper border border--table-border p-16 mt-24">
            ...
            <div class="full-width mt-8">
              <el-form @submit.native.prevent>
                <el-form-item :error="tempItem.brandErr" class="mb-0">
                  <template #label>
                    <div class="font-12">
                      Nama Brand
                    </div>
                  ...
                  <el-autocomplete
                    v-model="tempItem.shopee_brand_name"
                    :fetch-suggestions="getSearchBrand" 
                ...
                <el-form-item 
                  ...
                  <div class="textfield">
                    <div v-if="item.format_type === 'NORMAL'">
                      <div v-if="item.input_type === 'DROP_DOWN' || item.input_type === 'COMBO_BOX'">
                        <el-select
                          ...
                          <el-option
                            ...
                            :key="opt.value_id"
                            :label="opt.display_value_name"
                            :value="opt"
                          />
                        </el-select>
                      </div>
                      <div v-if="item.input_type === 'TEXT_FILED'">
                        <div v-if="item.attribute_type === 'DATE_TYPE' || item.attribute_type === 'TIMESTAMP_TYPE'">
                          <el-date-picker 
                            v-model="item.value"
                            type="date" 
                            format="dd MMMM yyyy" 
                      ...
                      <div v-if="item.input_type === 'MULTIPLE_SELECT_COMBO_BOX'">
                        <el-select
                          v-model="item.value"
                          value-key="value_id"
                          multiple
                          collapse-tags
                          class="full-width"
                          @change="handleChangeInput(); item.error = null">
                          <el-option
                            v-for="opt in item.options"
                            :key="opt.value_id"
                            :label="opt.display_value_name"
...
export default {
  ...
  methods: {
        ...
        fetchRequiredForm(params, item.shopee_category_id).then(response => {
          ...
          this.tempItem.formRequired.map(i => {
            ...
            if (i.input_type === 'TEXT_FILED') {
              if (i.attribute_type === 'FLOAT_TYPE' || i.attribute_type === 'INT_TYPE') {
                i.text_type = 'number'
              } else if (i.attribute_type === 'STRING_TYPE' || i.attribute_type === 'ENUM_TYPE') {
                i.text_type = 'text'
              }
            }
            
            temp.map(j => {
              if (i.attribute_id === j.attribute_id) {
                if (i.input_type === 'TEXT_FILED' && (i.attribute_type === 'DATE_TYPE' || i.attribute_type === 'TIMESTAMP_TYPE' )) {
                  i.value = moment.unix(j.attribute_value_list[0].original_value_name).format('YYYY-MM-DD')
                } else if (i.input_type === 'TEXT_FILED' && i.attribute_unit.length) {
                  i.value = j.attribute_value_list[0].original_value_name
                  i.value_unit = j.attribute_value_list[0].value_unit
                } else if (i.input_type === 'DROP_DOWN' || i.input_type === 'COMBO_BOX') {
                  let selectedOpt = i.options.find(needle => needle.value_id === j.attribute_value_list[0].value_id);
                  i.value = selectedOpt
                } else if (i.input_type === 'MULTIPLE_SELECT_COMBO_BOX') {
                  let selectedOptions = []
                  i.options.map(k => {
                    j.attribute_value_list.map(l => {
                      if (k.value_id === l.value_id) {
                        selectedOptions.push(k)
                      }
                    })
                  })
                  i.value = selectedOptions
                } else {
                  i.value = j.attribute_value_list[0].original_value_name
                }
              }
    ...
    getCategorySearch(queryString, callback) {
      ...
      // fetchListCategoryShopee
      // recomendationShopee diganti shopee/item-category
    ...
    handleGetRequired (data) {
      ...
      this.tempItem.shopee_brand_id = ''
      this.tempItem.shopee_brand_name = ''
      let params = {
        shopee_shop_id: this.shopeeStore.shop_id
      }
      this.tempItem.formRequired = []
      fetchRequiredForm(params, data.shopee_category_id).then(response => {
        ...
        this.tempItem.formRequired.map(i => {
          ...
          if (i.input_type === 'MULTIPLE_SELECT_COMBO_BOX') {
            this.$set(i, 'value', [])
          }
        ...
        console.log('required', this.tempItem.formRequired)
        this.loadingRequired = false
        // this.handleFormValidation()
        this.$forceUpdate()
      }).catch(error => {
    ...
    getSearchBrand(queryString, callback) {
      // this.loadingRequired = true
      let params = {
        sort_column: 'name',
        sort_type: 'asc',
        search: queryString,
        shopee_shop_id: this.shopeeStore.shop_id,
        shopee_category_id: this.tempItem.shopee_category_id
      }
      getListBrandShopee(params).then(response => {
        for (let i of response.data.data) {
          i.value = i.name
        }
        this.searchBrandData = response.data.data
        console.log('tes', this.searchBrandData)
    ...
    handleFormValidation () {
      var countError = 0
      if (!this.tempItem.shopee_brand_name && !this.tempItem.shopee_brand_id) {
        this.tempItem.brandErr = 'Nama brand harus diisi'
        countError++
      }
    ...
    saveSingleProductBatch (push_to_shopee) {
      ...
      if (this.handleFormValidation()) {
        ...
        let att = []
        let logs = []
        data.formRequired.map(i => { // dari get attribute
          let attribute = {
          ...
          if (i.input_type === 'TEXT_FILED') {
            let attribute_value = {
              value_id: 0,
              original_value_name: i.value,
              value_unit: ''
            }
            
            if (i.attribute_unit.length) {
              attribute_value.value_unit = i.value_unit
            } else if (i.attribute_type === 'DATE_TYPE' || i.attribute_type === 'TIMESTAMP_TYPE') {
              attribute_value.original_value_name = moment(i.value).unix()
              delete attribute_value.value_unit
            } else {
              delete attribute_value.value_unit
            }
            attribute.attribute_value_list.push(attribute_value)
          } else if (i.input_type === 'MULTIPLE_SELECT_COMBO_BOX') {
            i.value.map(j => {
              attribute.attribute_value_list.push(j)
            })
          } else {
            attribute.attribute_value_list.push(i.value)
          }

          if (i.is_mandatory || 
            (i.value instanceof Array && i.value.length) ||
            (i.value instanceof Object && Object.keys(i.value).length) ||
          ...
          // } else  if (i.input_type === 'TEXT_FILED' && i.attribute_type === 'DATE_TYPE') {
          //   attribute.value = moment(i.value).unix()
          // } else {
          //   i.value.map(j => {
          //     attribute.attribute_value_list.push({
          //       value_id: j.value_id
          //     })
          //   })
          // }
    ...
    handleChangeInput () {
      // console.log('@change', this.tempItem.formRequired)
      this.$forceUpdate()
    },

    handleSelectBrand (data) {
      this.tempItem.shopee_brand_id = data.shopee_brand_id
      this.tempItem.shopee_brand_name = data.name
      this.tempItem.brandErr = null
      this.$forceUpdate()
    },
