<template>
  <el-dialog
    ...
    <div v-if="dataProduct.length === 0" class="no-data-wrapper border border--grey p-32 align-center mt-16" style="display: flex; flex-direction: column;">
      <img src="/static/img/service-activation/tokopedia/no-product-tokped.png" class="no-data-image">
      <div class="text-center word-break" style="width: 360px" >Sip! Semua produk di Tokopedia telah dihubungkan dengan produk di toko Olseramu</div>
    </div>
    <template v-else>
      <!-- <el-table
        v-loading="loading"
      ...
      <div class="like-table-wrapper mt-16 border border--table-border">
        <div class="like-table-wrapper--item">
          ...
          <div class="font-bold font-14 " style="width: 20%">
            Stock
          </div>
        </div>
      </div>
      ...
      <div v-loading="loadingBulk" class="like-table-wrapper border border--table-border" >
        <div v-for="(item, idx) in dataProd" 
          ...
          <div class="flex-container full-width px-12 py-8 radius-50 border border--primary" style="max-width: 415px" :style="bgOlseradiv[idx]" @mouseover="setBg(idx)" @mouseleave="setBg1(idx)">
            <div class="font-14 full-width" >
              <div v-if="item.recomendation === null && item.pair === null" class="pointer" @click="handlePair(item)">{{ rootLang.select }} Product Olsera</div>
              <div v-else-if="item.recomendation !== null && item.pair === null" class="flex-container" >
                <div class="flex-container">
                  <el-avatar
              ...
              <div v-else-if="item.pair !== null && item.recomendation === null" class="flex-container" >
                <div class="flex-container">
                  <el-avatar
...
import { pairBulkProduct } from '@/api/thirdParty/tokopedia'
export default {
  ...
  methods: {
    ...
    handleSavePair () {
      ...
      this.dataChecked.map(i => {
        i.merchant_tokopedia_id = this.tokopediaActiveStore.id
      })
      // let data = {
