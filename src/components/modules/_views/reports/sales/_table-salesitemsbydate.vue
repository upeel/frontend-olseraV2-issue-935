<template>
  <div>
    <div class="summary">
      <h5>{{ $lang[langId].summary }}</h5>
      <bs-table
        :header="[
          $lang[langId].currency,
          lang.qty,
          lang.total + ' ' + lang.discount,
          lang.sales_amount,
          lang.product_cost,
          lang.profit
        ]">
        <tr
          v-for="(item, key) in summary"
          :key="key">
          <td>{{ item.currency_id }}</td>
          <td>{{ item.ftotal_qty }}</td>
          <td>{{ item.ftotal_discount }}</td>
          <td>{{ item.ftotal_amount }}</td>
          <td>{{ item.ftotal_cost_amount }}</td>
          <td>{{ item.ftotal_profit }}</td>
        </tr>
      </bs-table>
    </div>

    <hr/>

    <bs-table
      :header="[
        lang.order_no,
        lang.date,
        lang.sales_by,
        lang.item,
        lang.serial_no,
        lang.customer,
        lang.qty,
        lang.discount,
        lang.sales_amount,
        lang.product_cost,
        lang.profit
      ]">
      <tr
        v-for="(item, key) in data"
        :key="key">
        <td>
          <el-button v-if="$store.getters.webviewMode" type="text" @click="showDetailData(
              {path: item.status === 'Z' ? '/sales/completeorder/' + item.id : '/sales/openorder' + item.id,query: {'rpt': true, 'path': 'creditsalesdetails'}},
              {...item}
            )">
            {{ item.order_no }}
          </el-button>
          <router-link v-else :to="{path: item.status === 'Z' ? '/sales/completeorder/' + item.id : '/sales/openorder' + item.id, query: {'rpt': true, 'path': 'salesitemsbydate'}}">
            {{ item.order_no }}
          </router-link>
        </td>
        <td>{{ item.forder_date }}</td>
        <td>{{ item.sales_by_name }} ( {{item.order_source_name}} )</td>
        <td v-if="item.product_name">{{ item.product_name }} {{item.product_variant_name ? '(' + item.product_variant_name + ')' : ''}}</td>
        <td v-else>{{ item.product_combo_name }}</td>
        <td>{{ item.serial_no }}</td>
        <td>{{ item.customer_name }}</td>
        <td>{{ item.qty }}</td>
        <td>{{ item.discount }}</td>
        <td>{{ item.famount }}</td>
        <td>{{ item.fcost_amount }}</td>
        <td>{{ item.fprofit }}</td>
      </tr>
    </bs-table>

    <div v-if="total > 0" class="table-handler-bottom">
      <el-pagination
        :total="total"
        :page-sizes="[50, 100]"
        :page-size="15"
        :current-page.sync="currentPage"
        background
        layout="sizes, prev, pager, next"
        @current-change="handleChangePage"
        @size-change="handleChangeSizePage"
      />
    </div>
  </div>
</template>

<script>
export default {
  name: 'TableSalesitemsbydate',
  props: ['data', 'lang', 'summary', 'langId', 'total', 'currentPage'],
  methods: {
    showDetailData(path, data) {
      this.$emit('showdetaildata', path, data)
    },
    handleChangePage(page) {
      this.$emit('changePage', page)
      this.$emit('currentPage', this.currentPage)
    },
    handleChangeSizePage(page) {
      this.currentPage = 1
      this.$emit('changeSizePage', page)
    }
  }
}
</script>
