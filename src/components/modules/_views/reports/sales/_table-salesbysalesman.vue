    ...
    <template v-if="!isDetail">
      <div
        ...
        <el-button type="text" @click="showDetail(item)">
          <h4 class="font-bold">{{ item.sales_by_name }}</h4>
        </el-button>
        <div class="summary">
...
export default {
  ...
  props: ['data', 'langId', 'lang', 'total', 'params'],
  data () {
    return {
      isDetail: false,
      loading: false,
  ...
  methods: {
    ...
    getDetail() {
      this.loading = true
      this.params.sales_by_name = this.tempDetail.sales_by_name
    ...
    handleChangeSizePageDetail(page) {
