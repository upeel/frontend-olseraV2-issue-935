  ...
  <div>
    <div class="summary">
      ...
      <el-table :data="summary" border>
        ...
        <el-table-column min-width="140px" :label="$lang[langId].shipping_plus_tax" prop="ftotal_shipping_tax_amount" />
    ...
    <el-table :data="data" border>
      <el-table-column min-width="140px" :label="lang.order_no" prop="order_no">
        <template slot-scope="scope">
          <el-button v-if="$store.getters.webviewMode" type="text" @click="showDetailData(
              {path: '/sales/openorder/' + scope.row.id,query: {'rpt': true, 'path': 'creditsalesdetails'}},
              {...scope.row}
            )">
            {{ scope.row.order_no }}
          </el-button>
          <router-link v-else :to="{path: '/sales/openorder/' + scope.row.id,query: {'rpt': true, 'path': 'creditsalesdetails'}}">
            {{ scope.row.order_no }}
          </router-link>
        </template>
      </el-table-column>
      ...
      <el-table-column min-width="140px" :label="lang.sales_amount" prop="ftotal_amount" />
      <el-table-column min-width="140px" :label="$lang[langId].shipping_plus_tax" prop="fshipping_tax_amount" />
      <el-table-column min-width="140px" :label="lang.product_cost" prop="ftotal_cost_amount" />
      <el-table-column min-width="140px" :label="lang.profit" prop="fprofit_nett" />
