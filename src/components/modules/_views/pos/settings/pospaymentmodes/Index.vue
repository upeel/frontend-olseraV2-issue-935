<template>
  <div v-loading="loading">
    <div class="affix-wrapper flex-container flex-container--start flex-container--desktop">
      <div
        v-if="checkCustomPermission('settings/pospaymentmodes', 'store')"
  ...
  methods: {
    getData: function() {
      ...
      axios({
        ...
        this.dataTmp = response.data.data
    ...
    sortsChanged(data) {
      if (data.moved.newIndex === 0 || data.moved.oldIndex === 0) {
        this.data = this.dataTmp
      } else {
        if (this.checkCustomPermission('settings/pospaymentmodes', 'edit')) {
          this.saveSorts()
        }
      }
    },
