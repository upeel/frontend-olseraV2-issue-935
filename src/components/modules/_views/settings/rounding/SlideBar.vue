<template>
  <div
    ...
    <div class="flex-container">
      <div class="full-width mt-16">{{ showMin }}</div>
      <div class="mt-16">{{ showMax }}</div>
    </div>
      ...
      <template>
        ...
        <!-- <div ref="tooltip2" class="vue-slide-bar-always vue-slide-bar-tooltip-container">
          <div class="vue-slide-bar-tooltip-top vue-slide-bar-tooltip-wrap">
            <el-button class="mt-64" @click="edit">
              {{ val }}
              <svg-icon icon-class="edit-2" class="color-primary" />
            </el-button>
          </div>
...
export default {
  ...
  computed: {
    ...
    calculateMarginRightBottom () {
      let pos = (this.val/this.max) * 100
      let margin = ''
      if (pos < 10) { 
        margin = 'margin-left:' + 350 + '%'
      } else if (pos > 10) {
        margin = 'margin-right:' + pos + '%'
      }
  ...
  methods: {
    ...
    setValue (val, speed) {
      if (val === this.min) {
        let resetVal = this.min + 0.01
        this.val = resetVal
        this.syncValue()
      } else if (val === this.max) {
        let resetVal = this.max - 0.01
        this.val = resetVal
        this.syncValue()
      } else if (this.isDiff(this.val, val)) {
