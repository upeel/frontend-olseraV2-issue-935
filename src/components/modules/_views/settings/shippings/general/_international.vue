<template>
  <div>
    <el-card class="box-card with-left-poster" v-loading="loading">
      <div class="left-poster international">
        <i class="fa fa-globe fa-4x" aria-hidden="true"></i>
        <h3>{{ lang.international_shipping }}</h3>
        <p>{{ lang.info_international_shipping }}</p>
      </div>
      ...
      <div
        ...
        <div class="like-tr-wrapper">
          <div v-for="(item, key) in data" v-loading="loading" :key="key" class="like-tr clearfix">
            ...
            <div v-if="!item.is_custom" class="width-140 like-tr-item">
              <div class="editable-field">
                ...
                <el-input :disabled="!checkCustomPermission('settings/shippings', 'edit')" @change="save(item)" v-model="item.additional_fee" type="number" size="small" :placeholder="$lang[langId].input_number" />
              </div>
            </div>
    ...
    <el-dialog v-model="showDialog" v-loading="loadingDialog" :before-close="beforeCloseDialog" :visible.sync="showDialog" :title="lang.info_manage_shipping +' - '+ $lang[langId].custom">
...
const apiEndpoint = 'shippings/international'
...
export default {
  ...
  methods: {
    ...
    showShippingRates(item) {
      this.$router.push({
        path: '/settings/shippings/rates/' + item.courier_id,
        query: {
          international: 1
        }
      })
