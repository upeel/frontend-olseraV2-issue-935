  ...
  <div v-loading="loading" class="sales-detail">
    <section class="order-detail">
      <el-card class="box-card box-card--p-null">
        <div class="color-white px-16 flex-container" :class="openPurchaseData.is_paid === 1? 'color-green--bg' : 'color-danger--bg'">
          <div class="full-width flex-container">
            <div class="container-icon"><svg-icon :icon-class="openPurchaseData.is_paid === 1? 'check-circle' : 'x-circle'" /></div>
            <span>{{ openPurchaseData.is_paid === 1? rootLang.already_paid : rootLang.not_yet_paid }}</span>
          </div>
          <div class="flex-container">
            <span>{{ openPurchaseData.is_paid === 1? openPurchaseData.fpayment_date : '' }}</span>
            <el-switch
        ...
        <div class="card-body with-back-button">
          <el-button icon="el-icon-arrow-left" class="back-button" @click="backHandle"/>

          <div class="flex-container flex-container--wrap ml-8">
            <div style="font-size: 40px"><svg-icon icon-class="ico-olsera-segilima" /></div>
            <div style="flex-grow: 1;" class="ml-16">
              <h4 v-if="!openPurchaseData.purchase_no">-</h4>
              <h4 v-else class="font-bold">{{ openPurchaseData.purchase_no }}</h4>
          ...
          <el-card class="card-body">
            <div slot="header" class="flex-container flex-container--wrap">
              ...
              <div v-if="showEdit">
                ...
                <button-action-authenticated :permission="[permis, 'edit']" type="primary" v-if="!isEditing.due_date" size="mini" @click="handleEdit('due_date')" :disabled="computedAccess" icon="el-icon-edit">
                  {{ lang.edit }}
                </button-action-authenticated>
              </div>
            </div>
    ...
    return {
      ...
      statusSelect: [
        ...
        },
        {
          ...
          disabled: !this.checkCustomPermission('purchase/closedpurchases', 'store'),
        },
        {
          value: 'X',
          disabled: !this.checkCustomPermission('purchase/cancelledpurchases', 'store'),
